...
public class ClosestStars {
    ...
    public static List<Integer> kLaregestElements (int[] maxHeapArr, int k) {

	Pair<Integer, Integer> sth = new Pair<Integer, Integer>();
	List<Integer> resultList = new ArrayList<Integer>();
	PriorityQueue<Integer> maxHeapK = new PriorityQueue<Integer>();
	maxHeapk.add(maxHeapArr[0], 0);

	int i = 0;
	while (i < k) {
	    Pair<Integer, Integer> currMax = maxHeapk.poll();
	    resultList.add(Pair.getFirst());
	    maxHeapk.add(maxHeapArr[currMax.getSecond() * 2 + 1]);
	    maxHeapk.add(maxHeapArr[currMax.getSecond() * 2 + 2]);
	    i++;
	}
	return resultList;
    }
